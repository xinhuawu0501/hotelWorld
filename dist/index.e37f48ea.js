...
const loadCurHotelNearbyandQA = async function() {
    try {
        const id = state.curId;
        const hightlightPromise = fetch(`https://booking-com.p.rapidapi.com/v1/hotels/location-highlights?hotel_id=${id}&locale=en-gb`, (0, _config.options));
        const questionsPromise = fetch(`https://booking-com.p.rapidapi.com/v1/hotels/questions?locale=en-gb&hotel_id=${id}`, (0, _config.options));
...
const loadCurHotelFacAndReviews = async function() {
    try {
        const id = state.curId;
        const facilitiesPromise = fetch(`https://booking-com.p.rapidapi.com/v1/hotels/facilities?locale=en-gb&hotel_id=${id}`, (0, _config.options));
        const reviewsPromise = fetch(`https://booking-com.p.rapidapi.com/v1/hotels/reviews?sort_type=SORT_MOST_RELEVANT&locale=en-gb&hotel_id=${id}&language_filter=en-gb%2Cde%2Cfr&customer_type=solo_traveller%2Creview_category_group_of_friends`, (0, _config.options));
        const res = await Promise.allSettled([
            facilitiesPromise,
            reviewsPromise
        ]);
...
class ResultView extends (0, _viewDefault.default) {
    ...
    _generateMarkup(data) {
      ...
      <div class="details__info details__section">
        ...
        ${data.customerReviews ? ` <ul class="details__cusReviews details__section">
        ...
        ${data.facilities ? `<div class="details__facilities details__section">
        ...
        ${data.FAQ ? `<div class="details__FAQ details__section">
        <header>
